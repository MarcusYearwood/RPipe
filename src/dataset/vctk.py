...
class VCTK(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = (wave, speaker_id, acoustic_feature, text_tensor, ref_mel_tensor, ref_label, path)
        target = ref_mel_tensor
        input = {
                'id':  id, 'wave': data, 'speaker_id': target, 
                "acoustic_feature": acoustic_feature, "text_tensor": text_tensor,
                "ref_wave": ref_wave, "ref_mel_tensor": ref_mel_tensor, 
                "ref_label": ref_label, "path": path
            } #
        other = {k: torch.tensor(self.other[k][idx]) for k in self.other}
        input = {**input, **other}
        # if self.transform is not None: NOT SURE HOW I WANT TRANSFORMS TO WORK
        #     input = self.transform(input)
        return input
